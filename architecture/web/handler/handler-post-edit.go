...
func (m *MainHandler) PostEditHandler(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		...
		default:
			lg.Err.Printf("PostEditHandler: m.service.PostCategory.GetByPostID: %v\n", err)
	...
	case http.MethodPost:
		...
		case errors.Is(err, spost.ErrInvalidTitleLength) || errors.Is(err, spost.ErrInvalidContentLength):
			...
			default:
				lg.Err.Printf("PostEditHandler: havent got message for error: %s\n", err)
			...
			return
		case err != nil:
			lg.Err.Printf("PostEditHandler: m.service.Post.Update: %v\n", err)
			http.Error(w, "something wrong, maybe try again later", http.StatusInternalServerError)
			return
		}
		...
		case err == nil:
		case err != nil:
			lg.Err.Printf("PostEditHandler: m.service.PostCategory.DeleteByPostID: %v\n", err)
			http.Error(w, "something wrong, maybe try again later", http.StatusInternalServerError)
			return
		}
		...
		case errors.Is(err, scategory.ErrCategoryLimitForPost):
			...
			return
		default:
			lg.Err.Printf("PostEditHandler:  m.service.Category.AddToPostByNames: %s", err)
